...
14. For a Cody/build task, create the local n8n workflow draft and confirm `artifacts/<task_id>/build/n8n-workflow-draft.md` exists.
15. Restore the test task to its original state if needed.
16. Open the task card details and confirm latest task changes display.
17. Create a local approval request, then approve/defer/reject/cancel it from card details.
18. Confirm `tasks/_approvals/APP-####.json` and `tasks/_audit/approval-events.jsonl` update.
19. Run `node scripts\validate-gtm-tasks.js`.
...
`tasks/_audit/artifact-events.jsonl` records local artifact creation events only. Each line includes timestamp, task id, lane, files created, files skipped, actor when available, source, endpoint, and success. Artifact files are deterministic Markdown starters under `artifacts/<task_id>/`; the n8n draft endpoint is just a Cody/build artifact event and does not execute n8n.

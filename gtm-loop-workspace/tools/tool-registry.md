...
| n8n MCP | Cody/build-lane draft workflow executor surface | Draft | Inspect local availability, inspect workflow shape read-only when configured, generate local draft artifacts, prepare draft JSON, validate fake payloads | Activate workflows, create/update live production workflows, call production webhooks, write HubSpot/Gong/AirOps data, use real credentials, expose webhooks, schedule production automation | User | Available-local / draft-only / approval-gated | `integrations/n8n-mcp.md`, `executors/n8n-draft-executor.md`, `MCPS/N8N.json`, local `docker ps` on `2026-07-01` |
